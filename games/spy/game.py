...
ENGLISH_RULES = (
    "Citizens receive the same secret location. One player is secretly the Spy. "
    "Citizens ask careful questions to find the Spy, while the Spy blends in "
    "and tries to discover the location without being caught."
)
ARABIC_RULES = (
    "يحصل المواطنون على نفس الموقع السري، بينما يتم اختيار لاعب واحد سراً ليكون الجاسوس. "
    "يطرح المواطنون أسئلة ذكية لاكتشاف الجاسوس، ويحاول الجاسوس الاندماج ومعرفة الموقع دون أن ينكشف."
)
        ...
        for category in ("places", "items"):
            ...
            for item in raw_items:
                if isinstance(item, dict):
                    ...
                    if en:
                        entries.append({"en": en, "ar": ar or en})
    if entries:
        return entries
...
def lobby_embed(guild: discord.Guild, lobby: SpyLobby) -> discord.Embed:
    ...
    if lobby.finished and lobby.secret:
        embed.add_field(name="الكلمة/الموقع السري" if arabic else "Secret word/location", value=lobby.secret, inline=False)
...
class SpyLobbyView(SafeView):
    ...
    def _role_message(self, player_id: int) -> str | None:
        ...
        if player_id == self.lobby.spy_id:
            return "أنت الجاسوس! حاول معرفة الموقع دون أن تنكشف." if self.lobby.language == "ar" else "You are the Spy! Blend in and discover the location."
        return (
            f"أنت مواطن. الموقع السري هو: **{self.lobby.secret}**"
            if self.lobby.language == "ar"
            else f"You are a Citizen. The secret location is: **{self.lobby.secret}**"
        )
    ...
    async def _finish_match(self, guild: discord.Guild, *, outcome_error: bool = False) -> None:
        ...
        if lobby.vote_message:
            try:
                final_embed = vote_embed(guild, lobby, 0)
                final_embed.description = lobby.result_text or "Match finished."
