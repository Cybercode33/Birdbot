...
function renderPortalChoice(notice = "") {
  ...
  portalChoiceGrid.replaceChildren(
    ...
    renderPortalCard("Music System", "Open your server's Spotify player and control playback as an authenticated member.", "/?music=1", "portal-music-card"),
  );
}
...
function renderControlPanel() {
  ...
  cards.forEach(([title, description], index) => {
    ...
    if (isReady) {
      ...
      const open = () => index === 0 ? renderServerMessagePanel() : renderRolesPanel();
    ...
    card.append(
      ...
      textElement("span", "control-panel-card-status", index === 0 ? "Open composer" : index === 1 ? "Manage roles" : "Coming soon"),
    );
    grid.append(card);
  });
