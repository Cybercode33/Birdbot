...
async def spotify_callback(request: Request, code: str | None = None, state: str | None = None, error: str | None = None):
    ...
    store.save_spotify_account(
        ...
        str(profile.get("display_name") or profile.get("id") or "Spotify user"),
        str(access_token),
...
async def music_overview(guild_id: str, request: Request) -> dict[str, object]:
    ...
    if not account:
        return {"linked": False, "spotify_configured": spotify_is_configured(), "guild": bot_guild, "state": state, "playlists": [], "albums": [], "top_tracks": []}
...
async def music_action(guild_id: str, action: str, request: Request) -> dict[str, str]:
    ...
    if action in {"queue", "play"}:
        ...
        if not isinstance(track_id, str) or not SPOTIFY_ID.fullmatch(track_id):
            raise HTTPException(status_code=400, detail="Choose a valid Spotify track.")
        # Ask Spotify to resolve the track in the linked user's market. This
        # matters for search results outside their playlists: a track can be
        # globally searchable but unavailable in the default market, which
        # otherwise makes the dashboard report a misleading playback error.
