...
(function () {
  ...
  function trackKey(track) {
    return String(track && (track.id || `${track.artist || ""}\u0000${track.name || ""}`));
  }
  ...
  function mountMusicPanel({ root, guildId, requestJson, beginLoading, endLoading, initialOverview = null }) {
    ...
    function renderOverview() {
      ...
      heading.append(element("p", "music-kicker", "MUSIC SYSTEM"), element("h3", "music-title", "Your music, in voice"), element("p", "music-copy", "Link Spotify and control BirdBot's player without leaving your server dashboard."));
      const headerActions = element("div", "music-header-actions");
      const spotifyLink = document.createElement("a");
      spotifyLink.className = "spotify-button";
      if (overview.spotify_configured !== false) spotifyLink.href = `/music/spotify/login?guild=${encodeURIComponent(guildId)}`;
      else spotifyLink.classList.add("is-disabled");
      spotifyLink.textContent = overview.spotify_configured === false ? "Spotify setup required" : (overview.linked ? "Relink Spotify" : "Link Your Spotify Account");
      spotifyLink.target = "_self";
      if (overview.spotify_configured === false) spotifyLink.addEventListener("click", (event) => { event.preventDefault(); setStatus("Spotify integration is not configured on this server yet.", "error"); });
      ...
      if (!overview.linked) {
        ...
        empty.append(element("strong", "", "Connect your Spotify account"), element("p", "music-copy", "Your access is stored securely by BirdBot. Playlists and track metadata are only shown after you approve the Spotify link."));
