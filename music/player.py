...
class MusicPlayer:
    ...
    def _extract_audio(query: str) -> tuple[str, float, dict[str, str]]:
        ...
        # Render mounts uploaded Secret Files at /etc/secrets.  Keep the
        # environment variable override for custom filenames, but discover
        # the documented default automatically so a missing optional env var
        # does not silently disable YouTube authentication.
        if not cookies_file:
            default_secret = "/etc/secrets/youtube-cookies.txt"
            if os.path.isfile(default_secret):
                cookies_file = default_secret
